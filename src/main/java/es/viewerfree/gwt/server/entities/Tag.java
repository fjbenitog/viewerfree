...
@NamedNativeQueries({@NamedNativeQuery(name="findTagsByAlbum",query="SELECT  t.TAG_NAME,t.USER_ID_USER from viewerfree.VF_TAG t, viewerfree.VF_ALBUM_TAG a, viewerfree.VF_USERS u " +
		"where u.LOGIN = ? AND t.USER_ID_USER = u.ID_USER AND a.ALBUMS_NAME = ? AND t.TAG_NAME = a.VF_TAG_TAG_NAME AND u.ID_USER = a.vf_tag_user_ID_USER ORDER BY t.TAG_NAME ASC",resultClass=Tag.class),
		...
		"where u.LOGIN = ?1 AND t.USER_ID_USER = u.ID_USER AND a.ALBUMS_NAME = ?2 AND t.TAG_NAME = a.VF_TAG_TAG_NAME AND u.ID_USER = a.vf_tag_user_ID_USER) ORDER BY t.TAG_NAME",resultClass=Tag.class),
